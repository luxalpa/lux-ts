...
export class LuxParser extends Parser {
	...
	public statement(): StatementContext {
		...
		try {
			...
			case 3:
				...
				this._errHandler.sync(this);
				_la = this._input.LA(1);
				if ((((_la) & ~0x1F) === 0 && ((1 << _la) & ((1 << LuxParser.NUMBER) | (1 << LuxParser.LPAREN) | (1 << LuxParser.LSQUARE) | (1 << LuxParser.OPMULT))) !== 0) || _la === LuxParser.OPREF || _la === LuxParser.ID) {
					{
					this.state = 74;
					this.expr(0);
					}
	...
	public fnCallStatement(): FnCallStatementContext {
		...
		try {
			...
			this._errHandler.sync(this);
			_la = this._input.LA(1);
			if ((((_la) & ~0x1F) === 0 && ((1 << _la) & ((1 << LuxParser.NUMBER) | (1 << LuxParser.ENDL) | (1 << LuxParser.LPAREN) | (1 << LuxParser.LSQUARE) | (1 << LuxParser.OPMULT))) !== 0) || ((((_la - 32)) & ~0x1F) === 0 && ((1 << (_la - 32)) & ((1 << (LuxParser.OPMOD - 32)) | (1 << (LuxParser.OPREF - 32)) | (1 << (LuxParser.OPSKIP - 32)) | (1 << (LuxParser.ID - 32)))) !== 0)) {
				{
				this.state = 108;
				this.fnCallParams();
				}
	...
	public scope(): ScopeContext {
		...
		try {
			...
			this._errHandler.sync(this);
			_la = this._input.LA(1);
			if ((((_la) & ~0x1F) === 0 && ((1 << _la) & ((1 << LuxParser.NUMBER) | (1 << LuxParser.OPAT) | (1 << LuxParser.LPAREN) | (1 << LuxParser.LSQUARE) | (1 << LuxParser.OPMULT))) !== 0) || ((((_la - 37)) & ~0x1F) === 0 && ((1 << (_la - 37)) & ((1 << (LuxParser.OPREF - 37)) | (1 << (LuxParser.RETURN - 37)) | (1 << (LuxParser.IF - 37)) | (1 << (LuxParser.BREAK - 37)) | (1 << (LuxParser.FOR - 37)) | (1 << (LuxParser.ID - 37)))) !== 0)) {
				{
				this.state = 184;
				this.statement();
				this.state = 190;
...
export class DerefExprContext extends ExprContext {
	public OPMULT(): TerminalNode { return this.getToken(LuxParser.OPMULT, 0); }
	public expr(): ExprContext {
		return this.getRuleContext(0, ExprContext);
	}
	constructor(ctx: ExprContext) {
